...
DSPY_TUTORIAL_DIR = ROOT / "dspy_tutorial"
    ...
    *,
    ...
    if word_count > max_words:
        return PromptGenerationResult(
            ...
            sanitized_text=fallback_prompt,
            used_fallback=True,
            rejection_reason="too_long_after_cleanup",
        )
...
def _estimate_token_count_from_text(text: str) -> int:
    """Cheap conservative token estimate for chat preflight.

    Uses chars/token ~= 3.2 to avoid underestimating long prompts with markup.
    """
    if not text:
        return 0
...
def build_compact_candidate_prompt_request(
    *,
    seed_prompt: str,
    claim: str,
    ...
    return (
        "Write a complete replacement instruction prompt for a frozen fact-checking agent.\n\n"
        "Hard constraints:\n"
        "- Keep the same tool interface and ReAct setup.\n"
        "- The prompt must stay general-purpose and must not mention the current claim.\n"
        "- Improve search-vs-lookup decisions, trajectory inspection, redundancy avoidance, and stopping.\n"
        "- Prefer a concise, actionable prompt.\n\n"
        f"Exploration style: {exploration_profile['name']} - {exploration_profile['description']}\n\n"
        "Seed prompt:\n"
        f"<reference>\n{seed_prompt}\n</reference>\n\n"
        f"Current claim for context only: {claim}\n\n"
        "Return ONLY <prompt>...</prompt>.\n<prompt>"
    )


def _prepare_policy_request(
    *,
    seed_prompt: str,
    claim: str,
    exploration_profile: Dict[str, str],
    ...
    max_context = _get_model_context_size(model)
    reserve = CHAT_TEMPLATE_TOKEN_SLACK + 32

    primary = build_candidate_prompt_request(
        seed_prompt=seed_prompt,
        claim=claim,
        exploration_profile=exploration_profile,
    )
    primary_input_est = _estimate_token_count_from_text(primary)
    primary_available = max(32, max_context - primary_input_est - reserve)
    primary_max_tokens = min(requested_max_tokens, primary_available)

    if primary_available >= max(64, min(128, requested_max_tokens)):
        return primary, primary_max_tokens, "full"

    compact = build_compact_candidate_prompt_request(
        seed_prompt=seed_prompt,
        claim=claim,
        exploration_profile=exploration_profile,
    )
    compact_input_est = _estimate_token_count_from_text(compact)
    compact_available = max(32, max_context - compact_input_est - reserve)
    compact_max_tokens = min(requested_max_tokens, compact_available)
    return compact, compact_max_tokens, "compact"
...
def build_candidate_prompt_request(
    *,
    ...
    return (
        ...
        "- Aim for roughly 120-220 words unless the profile strongly benefits from a shorter prompt.\n"
...
def _build_runtime(
    *,
    ...
    try:
        ...
        hover_tools_dir = ROOT / "mellea_hover_baseline" / "src"
...
class PromptOptimizationAgent(LitAgent[Dict[str, Any]]):
    def __init__(
        ...
        *,
        ...
        min_generated_prompt_words: int,
        max_generated_prompt_words: int,
        ...
        self.min_generated_prompt_words = min_generated_prompt_words
        self.max_generated_prompt_words = max_generated_prompt_words
    ...
    def rollout(
        ...
        prompt_request, rollout_max_tokens, request_style = _prepare_policy_request(
            seed_prompt=self.seed_react_instructions,
            claim=task["claim"],
            exploration_profile=exploration_profile,
            requested_max_tokens=self.policy_max_tokens,
            model=llm.model,
        )
        print(
            f"[rollout {rollout.rollout_id}] policy request style={request_style} max_tokens={rollout_max_tokens}",
            flush=True,
        )
        ...
        prompt_generation = _sanitize_generated_prompt(
            ...
            task["claim"],
            min_words=self.min_generated_prompt_words,
            max_words=self.max_generated_prompt_words,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--wiki-dir",
        default=str(ROOT / "dspy_tutorial" / "wiki2017"),
        help="Path to the Wikipedia corpus directory.",
    )
    parser.add_argument(
        "--program-json",
        default=str(ROOT / "dspy_tutorial" / "mellea_tutorial" / "programs" / "dspy_optimized" / "best_program.json"),
        help="Optional DSPy program JSON used for fixed demos and extraction instructions.",
    )
    parser.add_argument("--n-train", type=int, default=16, help="Number of HoVer training examples.")
    parser.add_argument("--n-dev", type=int, default=4, help="Number of HoVer validation examples.")
    ...
    parser.add_argument("--policy-max-tokens", type=int, default=160, help="Max tokens for full-prompt generation. Keep this modest to avoid PPO memory spikes and context overflows.")
    parser.add_argument("--min-generated-prompt-words", type=int, default=40, help="Reject generated prompts shorter than this many words.")
    parser.add_argument("--max-generated-prompt-words", type=int, default=260, help="Reject generated prompts longer than this many words to keep rollouts stable.")
    parser.add_argument(
        "--max-policy-edits",
        type=int,
        default=2,
...
def build_output_dir(args: argparse.Namespace) -> Path:
    if args.output_dir:
        ...
        output_dir = ROOT / "lightning" / "runs" / f"prompt_opt_full_prompt_safe_{stamp}"
...
def main() -> int:
    ...
    print("(full prompt-string policy over a seed prompt, memory-safe variant)")
    print("=" * 60)
    ...
    run_config = {
        ...
        "min_generated_prompt_words": args.min_generated_prompt_words,
        "max_generated_prompt_words": args.max_generated_prompt_words,
    ...
    max_prompt_length, max_response_length = _compute_safe_verl_token_budgets(
        ...
        requested_response_length=min(args.policy_max_tokens, 256),
    )
